...
def git_status():
    """Checks the local repository Git status."""
    import subprocess
    try:
        # Get remote URL
        remote_res = subprocess.run(["git", "remote", "get-url", "origin"], capture_output=True, text=True)
        remote_url = remote_res.stdout.strip() if remote_res.returncode == 0 else "https://github.com/Chun-Chieh-Chang/PPOV-Extractor"
        ...
        changes = status_res.stdout.splitlines() if status_res.returncode == 0 else []
        ...
        return jsonify({
            ...
            "changes": [c.strip() for c in changes[:5]]
        })
    except Exception as e:
        return jsonify({"success": False, "message": str(e)})
...
def git_push():
    """Executes verification and pushes to GitHub."""
    import subprocess
    commit_msg = request.json.get("commit_msg", "update: automated commit from PPOV Extractor")
    if not commit_msg:
        return jsonify({"success": False, "message": "Commit 訊息不能為空"})
        
    try:
        # 1. 執行軟體確效測試
        test_res = subprocess.run([sys.executable, "verify_extraction.py"], capture_output=True, text=True)
        if test_res.returncode != 0:
            return jsonify({
                "success": False,
                "stage": "validation",
                "message": "軟體確效驗證失敗！推送程序已終止。",
                "details": test_res.stdout + "\n" + test_res.stderr
            })
        ...
        if status_res.stdout.strip():
            # 設定預設使用者資訊以防未設定時報錯
            subprocess.run(["git", "config", "user.name", "Wesley Chang"], check=True)
            subprocess.run(["git", "config", "user.email", "wesley@mouldex.com"], check=True)
            subprocess.run(["git", "commit", "-m", commit_msg], check=True)
            
        # 3. 推送到遠端
        env = os.environ.copy()
        env["GIT_TERMINAL_PROMPT"] = "0"  # 避免未授權時掛起，直接報錯
        push_res = subprocess.run(["git", "push", "origin", "main"], env=env, capture_output=True, text=True)
        
        if push_res.returncode != 0:
            # 如果 main 分支推送失敗，試著改為推送到 master
            push_res_master = subprocess.run(["git", "push", "origin", "master"], env=env, capture_output=True, text=True)
            if push_res_master.returncode != 0:
                return jsonify({
                    "success": False,
                    "stage": "push",
                    "message": "Push 失敗，請確認您已在本機設定 GitHub 憑證登入授權。",
                    "details": push_res.stderr + "\n" + push_res_master.stderr
                })
                
        return jsonify({
            "success": True,
            "message": "🎉 確效通過，且代碼已成功推送至 GitHub 倉庫！"
        })
    except Exception as e:
        return jsonify({"success": False, "message": str(e)})
