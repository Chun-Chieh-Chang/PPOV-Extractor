...
### 2026-05-27：v1.3.0 查檢數據即時輸入功能
* **需求背景 (Context)**：查檢人員希望能直接在 Web UI 上輸入查檢數據（實際值、查檢值、簽名欄位），而非僅能列印空白表單後手寫。
* **設計決策 (Design Decisions)**：
  1. **編輯模式切換**：採用「✏️ 輸入查檢數據 ↔ ✅ 確認儲存」雙態按鈕設計，避免誤觸修改。
  2. **輸入框風格**：Material Design 底線式輸入框（無邊框，聚焦時底線變色），保持表格簡潔。
  3. **快捷鍵支援**：Enter 鍵自動跳轉至下一個輸入框，最後一個欄位按 Enter 自動儲存。
  4. **數據暫存**：使用 `state.inspectionData[partNo]` 以品號為 key 暫存，切換品號後數據保留。
  5. **匯出整合**：前端 ExcelJS 與後端 openpyxl 兩條匯出路徑均已整合 inspectionData 寫入。
  6. **AI 預留**：新增「📷 AI 照片匯入」disabled 按鈕，附帶 tooltip 提示功能開發中。
* **可編輯欄位範圍**：
  - 製程參數表的「實際值」欄位 (10 個)
  - 參考規格參數表的「查檢值」欄位 (4 個)
