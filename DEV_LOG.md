...
* **歷史基準繼承與防禦性開發元規則 (2026-08-02 Meta-Rule)**：
  * **原則 (First Principles)**：凡專案中已有經過確效與測試之歷史變更（例如提交 `a0e732a` 建立之檔名唯一鍵與多模具防衝突架構、`config.json` 50+ 欄位標準名），在後續重構或增加新功能時，必須 100% 繼承現有架構，嚴禁擅自替換為簡化 Dummy 資料或破壞現有欄位命名對應。
  * **矯正與預防措施 (CAPA)**：
