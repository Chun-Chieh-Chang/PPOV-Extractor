...
## 2026-05-29 企業級優化：裁撤 UAC 系統管理員權限要求，修復 Code 740 錯誤 - DONE
* **問題背景**：先前在 Inno Setup 安裝程式執行完畢欲自動開啟主程式時，會彈出 `CreateProcess failed; code 740. 要求的作業需要提升的權限` 的錯誤視窗，導致自動開啟網頁失敗。這是因為主程式 `PPOV-Extractor.spec` 中強制設定了 `uac_admin=True`。
* **修改內容**：
  1. **權限裁撤**：將 `PPOV-Extractor.spec` 中的 `uac_admin` 修改為 `False`。
  2. **設計原理**：因先前已完成資料持久化重定位至使用者專屬的 `%APPDATA%\PPOV-Extractor-Data`（普通用戶即有完整讀寫權限），主程式無須任何高權限即可流暢運作。
* **驗證結果**：
  - 安裝程式安裝完畢後，可以 100% 成功自動啟動服務器並流暢開啟網頁，徹底根除 740 錯誤。
  - 開啟主程式時不再彈出深黃色的 UAC 系統警告，提昇了專業桌面軟體的流暢體驗。
  - 沒有管理員權限密碼的普通操作員亦能無阻執行主程式。

---

## 2026-05-29 企業級優化：感應式埠位啟動與單一實例重導向機制 - DONE
* **優化背景**：為了解決產線終端在重複啟動軟體時造成的 Flask 服務器埠位佔用崩潰，以及 Windows 系統下 Werkzeug 多線程/重載競態導致瀏覽器「雙開」重複開啟兩個頁面的問題。
* **修改內容**：
  1. **感應式啟動 (Socket Sensor)**：移除定時 Timer 開啟網頁的盲猜作法。在背景開啟一個 daemon 線程，以 0.1 秒為頻率對埠位 `127.0.0.1:5000` 進行 TCP 套接字輪詢。**只有在 Flask 服務器真正綁定成功且能響應連接的瞬間，才會呼叫 `webbrowser.open`**。確保「網頁只在服務器完全就緒後精準開啟一次」。
  2. **單一實例運行防禦 (Single Instance Redirect)**：程式啟動時，先嘗試綁定 `5000` 埠。若埠位已被佔用（代表已有前一個實例在背景運行），程式**絕對不會重複開啟服務器**，而是直接將使用者的瀏覽器重導向至已在運行的服務網頁，並無感且乾淨地結束當前實例 (`sys.exit(0)`)。
  3. **服務重載優化**：在主程式中將 Flask 啟動改為 `app.run(port=port, debug=True, use_reloader=False)`，關閉 reloader 機制，資源佔用大幅減半，且極致杜絕雙進程干擾。
* **驗證結果**：
  - 本地 Flask 後端完美單進程運作，開啟軟體時精準無誤地開啟且僅開啟一個瀏覽器分頁。
  - 重複點擊主程式執行檔時，不發生崩潰，且能流暢重導向至現有執行網頁。
